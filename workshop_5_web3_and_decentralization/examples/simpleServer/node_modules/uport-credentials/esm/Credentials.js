...
var Types;
(function (Types) {
    Types["DISCLOSURE_REQUEST"] = "shareReq";
    Types["DISCLOSURE_RESPONSE"] = "shareResp";
    Types["TYPED_DATA_SIGNATURE_REQUEST"] = "eip712Req";
    Types["VERIFICATION_SIGNATURE_REQUEST"] = "verReq";
    Types["ETH_TX_REQUEST"] = "ethtx";
    Types["PERSONAL_SIGN_REQUEST"] = "personalSigReq";
})(Types || (Types = {}));
/**
...
class Credentials {
    ...
    signJWT(payload, expiresIn) {
        ...
        return createJWT(payload, {
            issuer: this.did,
            signer: this.signer,
            alg: this.did.match('^did:uport:') || isMNID(this.did)
                ? 'ES256K'
                : 'ES256K-R',
            expiresIn
        });
     ...
     */
    createVerificationSignatureRequest(unsignedClaim, { aud, sub, riss, callbackUrl, vc, rexp, expiresIn }) {
        return this.signJWT({
            unsignedClaim,
            sub,
            riss,
     ...
     */
    async createTypedDataSignatureRequest(typedData, { from, net, callback } = {}) {
    ...
    async processDisclosurePayload({ doc, payload }) {
        ...
        const { own = {}, capabilities = [], aud, // ignored
        req, // ignored
        iat, // ignored
        exp, // ignored
        type, nad: mnid, dad: deviceKey, iss: did, boxPub, verified, ...rest } = payload;
        ...
        if (verified) {
            ...
            unfiltered.forEach(item => {
                if (item) {
                    verifications.push({ ...item.payload, jwt: item.jwt });
                }
            });
     ...
     */
    async authenticateDisclosureResponse(token, callbackUrl) {
        ...
        if (payload.req) {
            ...
            const request = challengeReq.payload;
